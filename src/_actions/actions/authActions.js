...
export const LoginAction =
  (credentials, setSubmitting, moveTo, t) => async (dispatch) => {
    try {
      await axios
        .post(`${currentUrl}auth`, credentials)
        .then((response) => {
...
export const SignupAction =
  (credentials, setSubmitting, moveTo, t) => async (dispatch) => {
    try {
      //Call Backend
      await axios
        .post(`${currentUrl}signup`, credentials)
        .then(async (response) => {
          if (response.data.status === "Failed") {
            setSubmitting(false);
            Toast.show(t(response.data.message), {
...
export const ForgotPasswordAction =
  (credentials, setSubmitting, moveTo, t) => async (dispatch) => {
    try {
      //Call Backend
      await axios
        .post(`${currentUrl}forget-password`, credentials)
        .then(async (response) => {
          if (response.data.status === "Failed") {
            setSubmitting(false);
            Toast.show(t(response.data.message), {
...
export const ResetPasswordAction =
  (values, setSubmitting, moveTo, route, t) => async (dispatch) => {
    try {
      ...
      await axios
        .post(`${currentUrl}reset-password`, { values, email })
        .then((response) => {
          if (response.data.status === "Failed") {
            Toast.show(t(response.data.message), {
              duration: 10000,
